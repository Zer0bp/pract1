...
size_t minr(struct Record* rd)
{
    size_t res = 0;
    unsigned int mn = 30;
    for (size_t i = 0; i < 3; i++)
    {
        if (rd[i].founded.month < mn)
            res = i;
    }
    return res;
}
size_t maxr(struct Record* rd)
{
    size_t res = 0;
    unsigned int mx = -1;
    for (size_t i = 0; i < 3; i++)
    {
        if (rd[i].founded.month > mx)
            res = i;
    }
    return res;
}
int main()
{
    ...
    Draw(rdcs);
    size_t mnidx = minr(rdcs);
    size_t mxidx = maxr(rdcs);
    swap(rdcs[mnidx], rdcs[mxidx]);
    Draw(rdcs);
    return 0;
}
